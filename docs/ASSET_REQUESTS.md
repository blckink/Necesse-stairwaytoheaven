...
| ~~Tree of Plenty~~ **DONE 2026-09-09** — own world sheet, item icon and object ID | `treeofplenty` | ~~`objects/appletree.png`~~ **ours** | **256×640** | Object/tree sheet. Exact 2×5 template retained. |
| ~~Giant Fig Tree~~ **DONE 2026-09-09** — own world sheet, item icon and object ID | `giantfigtree` | ~~`objects/bananatree.png`~~ **ours** | **256×640** | Object/tree sheet. Exact 2×5 template retained. |
| ~~Knowledge Tree~~ **DONE 2026-09-09** — own landmark sheet, item icon and object ID | `knowledgetree` | ~~`objects/dryadtree.png`~~ **ours** | **384×512** | Exact 3×4 tree/particle layout retained. |
| ~~Paradise Fern~~ **DONE 2026-09-09** — own object ID and sheet | `paradisefern` | `objects/paradisefern.png` | **256×32** | Eight separate 32px variants per sheet. |
| **Giant Monstera** — own object ID and variant sheet (reopened 2026-09-09) | `giantmonstera` (currently aliases `swampgrass`, no own ID registered); `adamsvine` also stays on `swampgrass` | `objects/swampgrass.png` | **256×32** | **Eight** separate 32px variants. The engine derives the count as `width / 32` (`GrassObject.loadTextures`) and picks one uniformly per tile, so **every empty cell is a plant that renders as nothing in game**. Each variant must be a whole plant, centred in its own 32px cell, with **≥1px transparent gutter left and right**, grounded on the cell floor (art reaching the bottom edge, not floating in the middle), **300–600 opaque pixels** — vanilla's own grass strips run 156–516. The first delivery on 2026-09-09 was rejected: cells were `[0, 0, 216, 267, 261, 198, 0, 0]`, i.e. four empty, and the filled ones were 11–26px clumps floating at mid-height and crossing the cell seams. Checked by `tools/variant_strip_audit.py`, which `asset_intake.py` does not do. |
