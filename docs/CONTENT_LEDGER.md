...
|---|---|---|
| `salonchair` | object | Friseurstuhl — hydraulischer Salonsessel auf Vanillas `ChairObject`, vier Blickrichtungen, ein Siedler kann darin sitzen. `HappinessObject` (RARE, 40 Punkte). Nur bei der Stylistin zu kaufen, kein Rezept. |
| `salonmirror` | object | Beleuchteter Friseurspiegel — haengt auf Vanillas `PaintingObject`-Rahmen an der Wandflaeche, eine Zelle je Wandrichtung, Seitenansichten schmal. Leuchtet (Stufe 100, fast weiss) und zaehlt als `lights`. `HappinessObject` (EPIC, 50 Punkte). Nur bei der Stylistin. |
| `salonsign` | object | Beleuchtetes Friseurladen-Schild — wird wie eine Lampe an die Wand gehaengt (`WallTorchObject`), an/aus, vier Anbaurichtungen, Schere als Zeichen, rosé-magenta Licht. `HappinessObject` (RARE). Nur bei der Stylistin. |
| `saloncashregister` | object | Friseurladen-Kasse — messingne Registrierkasse auf Vanillas `TableDecorationObject`, steht auf dem Tisch, vier Blickrichtungen. `HappinessObject` (RARE). Nur bei der Stylistin. |
