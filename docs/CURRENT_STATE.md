...
**Updated:** 2026-09-02 — Veil gate, four settlers, Crooked Beyond, Ghost and
Steinfeld are integrated; Eden's generated core is under final verification.

## Architecture in one screen

Six extra dimensions are registered through
`LevelIdentifier.IDENTIFIER_TO_DIMENSION`: **Skyreach** (`skyreach2`, +1),
**Eden** (`eden2`, +2), **Steinfeld** (`steinfeld2`, +3), **The Veil**
(`veil2`, −3), **Ghost Realm** (`ghost2`, +4), and **Crooked Beyond**
(`crooked2`, +5). All six are `BiomeGeneratorStackLevel`s that stream regions,
so they are effectively infinite and generate lazily. Eden is generated by the
integration command; Steinfeld, Ghost and Crooked compile and register but
their maps are not yet probed.
