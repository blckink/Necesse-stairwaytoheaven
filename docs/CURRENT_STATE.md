...
**Branch:** `codex/realms-integration` (published to `master` in green batches)
**Updated:** 2026-09-02 — **the six dimensions were collapsed into ONE PLANE**
(`docs/PLAN_ONE_PLANE.md`). Every realm is now a band of the sky level.

## Architecture in one screen

**One extra dimension**, registered through
`LevelIdentifier.IDENTIFIER_TO_DIMENSION`: **Skyreach** (`skyreach2`, +1). It
is a `BiomeGeneratorStackLevel` that streams regions, so it is effectively
infinite and generates lazily.

Every realm is a **band of that one level**, chosen by `worldgen/RealmDepth`
from distance to the Old Warden Spire — `docs/WORLD_DESIGN.md` §3's overlapping
biome weights, not hard zones:

| realm | depth | tiles from the spire |
|---|---|---|
...
(The rows are the trapezoids in `RealmDepth.BANDS`: foot to foot, so the
overlaps between neighbours are what §3 asks for. The peaks — where a realm is
unambiguously itself — are the narrower ranges §3's own table prints.)

`edenlevel`, `steinfeldlevel`, `ghostlevel`, `crookedlevel` and `veillevel`
were registered for one day and are **gone**. Nothing about their content
changed: each realm's `*TerrainPainter` became a band painter that
`SkyTerrainPainter.describeTile` calls, their biomes are painted into the sky
level's own biome layer, their POI presets and guard packs are gated on the
region's realm, and their settlers are placed by `SkyLevel`. The Veil's own
ground moved with them: Gloomfen and Ashen Reach are the fen inside the Ghost
band, the Beetlefreak Hollows are Crooked Beyond's own wrong ground
(`WORLD_DESIGN` §41.5). **Hell is not built** (§17–23); its band is painted as
the far end of Crooked until it is.
