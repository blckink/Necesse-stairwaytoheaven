...
| **Adventurer's Journal** (item) | anywhere | Right-click in the inventory (or use from the hotbar): the server builds that player's journal — per realm, every story step/quest with status, giver and where they live, region key / Summoning Stones / boss, residents met, places found, lore unlocked — and sends it (`journal/PacketJournalOpen`) to a window. Handed out on a player's first moment in the Skyreach, and once, retroactively, to anyone in an old save who has started the story; the Warden sells a replacement (100). Server side VERIFIED [run] by `/swhjournal` in `integration_test.sh`; the window itself is HYPOTHESIS until opened in game. |
...
| tool | state |
|---|---|
| `/swhshowroom build/goto/list/check/clear/export/tp` — 70 exhibits (33 POI kinds, 13 standalone presets, the whole Spire Village as `spirevillage`, per-realm tile/wall/object galleries and ground samples) at sky tile 20000,20000, spawn-free biome, lit | VERIFIED [run] on the dedicated server: built twice + checked, `missing=0`, `mobs=0` |
