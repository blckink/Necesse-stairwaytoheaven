...
/**
 * The Crooked-side door, leading back up into the Skyreach.
 *
 * <p>Functional twin of the Veil's returning rift and of the sky-side return
 * stairway: auto-placed on first arrival, restricted to this realm, and it
 * cleans up its counterpart when broken exactly the way vanilla ladder pairs do
 * — which is what stops a player from stranding themselves by mining the door
 * they came through and leaving a live one-way portal behind on the other side.
 *
 * <p><b>Art:</b> the mod's own {@code objects/veilriftup.png}, the returning
 * half of the same pair {@link CrookedDoorObject} borrows. Recorded as a
 * stand-in for §15's red door in {@code docs/realms/crooked.md}.
 */
public class CrookedSideDoorObject extends GameObject {

    public GameTexture texture;

    public CrookedSideDoorObject() {
        this.mapColor = new Color(150, 60, 180);
    ...
    public GameMessage getNewLocalization() {
        return new LocalMessage("object", "crookeddoor");
    }
    ...
    public LootTable getLootTable(Level level, int layerID, int tileX, int tileY) {
        // Breaking either side yields whatever the down-side yields, which is
        // nothing: the pair is registered unobtainable, because a door into a
        // tier-10 realm is not a thing to be carried around in a backpack.
        return ObjectRegistry.getObject(SkyRegistry.crookedDoorDownID)
