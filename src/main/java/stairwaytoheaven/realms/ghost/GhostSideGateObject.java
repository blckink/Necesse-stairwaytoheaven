...
/**
 * The Aftergarden side of the Ghost Gate: the way back to the living world.
 *
 * <p>Functional twin of the Veil's return rift and of the sky's return
 * stairway, and written from the same file so the three behave identically:
 * auto-placed on first arrival, refuses to be placed anywhere but the Ghost
 * Realm, and cleans up its counterpart when broken so a player cannot leave a
 * one-way door behind.
 *
 * <p>Its sheet is borrowed — {@code objects/veilriftup.png}, the mod's existing
 * return-rift art. Registered unobtainable, so no item icon exists or is
 * needed; breaking either half yields whatever the far half's loot table says,
 * which for an unobtainable pair is nothing.
 */
public class GhostSideGateObject extends GameObject {

    public GameTexture texture;

    public GhostSideGateObject() {
        this.mapColor = new Color(84, 190, 176);
    ...
    public GameMessage getNewLocalization() {
        return new LocalMessage("object", "ghostgate");
    }
    ...
    public LootTable getLootTable(Level level, int layerID, int tileX, int tileY) {
        return ObjectRegistry.getObject(GhostRealm.gateDownID)
    ...
    public String canPlace(Level level, int layerID, int x, int y, int rotation, boolean byPlayer,
        ...
        return !level.getIdentifier().equals(SkyRegistry.GHOST_IDENTIFIER)
                ? "invalidlevel"
